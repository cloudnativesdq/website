      ...
      <div className="container px-6 relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-6">
          {organizers.map((org, i) => (
            <OrganizerCard key={org.name + i} organizer={org} />
          ))}
        </div>
      </div>
...
function OrganizerCard({ organizer }: { organizer: Organizer }) {
  const [imgSrc, setImgSrc] = useState(organizer.image || links.organizerFallbackImage);
    ...
    >
      <div className="relative w-20 h-20 md:w-28 md:h-28 rounded-[1.5rem] md:rounded-[2rem] overflow-hidden mb-6 border-2 border-white/10 group-hover:border-primary/50 transition-all duration-500 rotate-3 group-hover:rotate-0 bg-muted">
        <Image
          ...
          onError={() => setImgSrc(links.organizerFallbackImage)}
        />
      </div>
      ...
      <div className="space-y-1">
        ...
        <p className="text-[10px] md:text-xs text-primary font-bold uppercase tracking-wider">{organizer.role}</p>
        <p className="text-[10px] text-muted-foreground font-medium line-clamp-1">{organizer.company}</p>
      </div>
    </a>
  );
}
